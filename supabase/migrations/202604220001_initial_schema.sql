-- Core schema for Gotfoth on Supabase
...
values
  ...
  ('marking-yaml', 'marking-yaml', false)
on conflict (id) do nothing;

-- RLS policy baseline: no direct client access to tables/buckets by default.
alter table public.papers enable row level security;
alter table public.boundaries enable row level security;
alter table public.settings enable row level security;

-- Authenticated read access can be granted later if desired.
drop policy if exists boundaries_read_auth on public.boundaries;
create policy boundaries_read_auth
...
drop policy if exists papers_read_auth on public.papers;
create policy papers_read_auth
on public.papers
for select
to authenticated
using (true);

-- Storage object access disabled for anon/authenticated users.
